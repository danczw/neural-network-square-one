...
class Optimizer_SGD:
    ...
    def update_params(self, layer):
        '''
        Momentum of parameter updates
            - Reason: Help to prevent getting stuck in local minima by passing
                through, further decreasing loss, by adding momentum
            - Momentum creates rolling average of gradients over some number
                of updates and uses this average with unique gradient at
                current step
            => Keeping "momentum" towards the global gradient descentdirection
        Adding momentum
            - Utilize momentum by setting a hyperparameter between 0 and 1,
                representing fraction of previous parameter update to retain
            - Subtract (adding the negative) the actual current gradient
            - Multiply by the learning rate (as before)
            => Update contains a portion of the gradient from preceding steps
                as momentum (i.e. direction of previous change) and a portion
                of current gradient
            - the bigger the role of momentum, the slower the update can
                change direction
        '''
        # If momentum is used
        if self.momentum:
...
# TODO: try different momentum values
