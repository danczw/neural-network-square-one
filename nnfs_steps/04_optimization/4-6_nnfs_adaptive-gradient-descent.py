...
class Optimizer_AdaGrad:
    ...
    Adaptive Gradient Descent optimizer
        - Institutes per-parameter learning rate rather than globally-shared rate
        - Normalizes updates made to parameters, as during training process,
            some weights can rise significantly while others not change much
        - Normalization is implemented by keeping a history of previous updates
        - The bigger the sum of updates (either positive of negative), the
            smaller updates are made futher in training
        => Lets less-frequently updated parameters keep-up with changes,
            effectively utilizing more neurons for training

        - This optimizer sometimes causes the learning rate to stall as updates
            become smaller with time
        - Therefore, this optimizer is not widely used except for some
            specific use cases
    '''
...
# TODO: try different epsilon values for adaptive gradient optimization
optimizer = Optimizer_AdaGrad(decay=1e-3, epsilon=1e-7)
