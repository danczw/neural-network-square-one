...
class Optimizer_RMSprop:
    ...
    Root Mean Square Propagation (RMSprop)
        - RMSprop calculates adaptive learning rate per parameter using a cache
            and also adds mechanism similar to SGD momentum
        - Helps to retain global direction of changes and slows changes
            in direction
        - Instead of continuous addition of squared gradients to cache
            (like AdaGrad), moving average of cache is used
        - Each update of cache retains part of cache and updates it with
            fraction of new, squared, gradient
        => cache content "moves" with data in time, and learnign rate
            does not stall
    '''
    ...
    def update_params(self, layer):
        ...
        '''
        Adding RMSprop - cache update
            - Rho is cache memory decay rate
            - Multiply rho with existing cache
            - Add fraction of new squared gradient
        
        Adding RMSprop - parameter update
            - Multiple (negative) learning rate by graidients (as in SGD)
            - Divide by cache, i.e. square historical gradients and
                add epsilon to avoid division by 0
            - epsilon as hyperparameter usually a small value
        '''
...
# TODO: try different hyperparameter for RMSprop optimization
optimizer = Optimizer_RMSprop(learning_rate=0.02, epsilon=1e-7, rho=0.999)
